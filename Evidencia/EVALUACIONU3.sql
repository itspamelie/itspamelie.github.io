...
/*Selecciona la cantidad de productos que nos ha comprado cada usuario*/
SELECT v.id_usuario, COUNT(vp.id_prod) AS cantidad_productos_vendidos FROM venta v INNER JOIN venta_productos vp ON v.id_venta = vp.id_venta GROUP BY  v.id_usuario;
/*Selecciona el total de venta por cada producto*/
SELECT p.id_prod, p.nombre, SUM(vp.cantidad) AS total_vendido FROM venta_productos vp INNER JOIN productos p ON vp.id_prod = p.id_prod INNER JOIN  venta v ON vp.id_venta = v.id_venta GROUP BY p.id_prod, p.nombre;
